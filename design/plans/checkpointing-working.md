...
**Prior-run wrap spans.** On resume, hydrate wraps the rehydrated checkpoint spans in a synthesized `span_begin / span_end` pair of `type="prior_run"`, `name="prior run N"`. The wrap is a sibling at the sample root (parent_id `None`) and contains one prior session's checkpoint spans plus their contents. Each resume adds exactly one new wrap; events.json after R resumes contains R sibling `prior_run` wraps followed by the current session's flat checkpoint spans (e.g. attempt 4 resuming a chain of attempt 1→2→3 produces `prior run 1` / `prior run 2` / `prior run 3` wraps, then `checkpoint 7` and onward as the current session fires).
