...
- **Validate.** `_validate_resume_state` raises `RuntimeError` if any
  ...
  `span_begin name="prior run 1" type="prior_run"`; `events[-1]` is
  a `span_end`; checkpoint span names sequential
  `checkpoint 1..N`; `prior_run` wrap names sequential
  `prior run 1..M`; every `span_begin` pairs with a matching
...
`f"prior run {next_n}"` where `next_n` is the count of existing
